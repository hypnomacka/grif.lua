...
local info_t = obex_fetch and obex_fetch() or { username = "Marlboro Purple", build = "Purple", discord = 0 }

local _VERSION = "Sakura"
local _LOCALLIB = info_t.username == "DEV"
...
local render do
	...
	_DPI.callback = function ()
		...
		if old ~= DPI then
			callbacks["sakura::render_dpi"]:fire(DPI)
			old = DPI
		end
	end
...
filesystem.create_directory("sakura", "ROOT_PATH")
...
sakura.print = function (...)
	printc(_AZAZI and "  \vazazeria\r  " or "  \vsakura\r  ", ...)
end

--

local debug = function (...)
	if _DEBUG then printc("  \vsakura\r  ", ...) end
end
...
local db = {
	...
	do
		local function automemo ()
			debug("autosave")
			client.fire_event("sakura::database_write")
			database.write(db.key, data)
...
pui.macros.p = "\aCDCDCD40â˘\r  "
pui.macros.sakura = colors.hex
pui.macros.sakurab = string.sub(colors.hex, 2, 7)
...
local menu, groups = {
	...
	set_visible = function (bool, aa)
		pui.traverse(refs.aa, function (r, path)
			if aa and sakura.build == "stable" and vars.antiaim.global.value and path[1] == "angles" then
				r:set_visible(false)
			else
	...
	header = function (group, text)
		local r = {}
		if text then r[#r+1] = group:label("\vâ˘\r  ".. text) end
		r[#r+1] = group:label("\a373737FFâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâž")
		return r
	end,
...
menu.main = {
	...
	global = groups.fakelag:checkbox("!!!!!SAKURA!!!!!"),
	bar = groups.fakelag:label("\f<silent>âžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâž"),
	tab = groups.fakelag:combobox("\n", table.distribute(menu.tabs, 2)),
	-- menu.space(groups.fakelag),
}

menu.misc = {
	overridden = groups.angles:label("Overridden by \vsakura")
}

menu.info = {
	user = groups.fakelag:label(" \f<silent>User   \v".. sakura.user.name),
	version = groups.fakelag:label( (sakura.build == "stable" and " \f<silent>Build:    \v%s" or " \f<silent>Version   \v%s â˘ %s"):format(sakura.version, sakura.build) ),
...
menu.general = {
	config = {
		...
		load = groups.angles:button("\f<sakura>Load", NILFN),
...
vars.visuals = {
	...
	crosshair = menu.feature(groups.angles:checkbox("Crosshair indicators"), function (parent)
		return {
			style = groups.angles:combobox("\nch_style", {"Classic", "Logo"}),
			logo = groups.angles:checkbox("Dont show build"),
		}, true
	end),
...
vars.antiaim = {
	...
	builder = {
		state = groups.angles:combobox("\vâ˘\r  State  \a373737FF----------------------------", table.distribute(antiaim.states, 2), nil, false),
...
do -- states
	...
	for i, v in ipairs(antiaim.states) do
		...
		--
		ctx.y_off	= new({id, "yaw", "offset"}, groups.angles:slider("Offset\f<_p>", -60, 60, 0, true, "Â°"))

		ctx[#ctx+1] = menu.space(groups.angles)
		ctx.mod		= new({id, "mod", "type"}, groups.angles:combobox("Modifier\f<_p>", {"None", "Jitter", "X-way", "Rotate", "Random", _AZAZI and "ĐżĐľŃĐ´ĐľĐťŃŃĐ°" or nil}))
		...
		ctx.ways = {} for w = 1, 7 do
			ctx.ways[w] = new({id, "way", w}, groups.angles:slider("\n"..w..id, -60, 60, 0, true, "Â°", 1, {[0] = "R"}))
			ctx.ways[w]:depend({ctx.mod, "X-way"}, ctx.w_m, {ctx.w_num, w, 7})
		end

		--
		ctx.m_d		= new({id, "mod", "degree"}, groups.angles:slider("Degree\f<_p>", -60, 60, 0, true, "Â°"))
		ctx.m_min	= new({id, "mod", "min"}, groups.angles:slider("Range \aCDCDCD60min/max\f<_p>", -60, 60, 0, true, "Â°"))
		ctx.m_max	= new({id, "mod", "max"}, groups.angles:slider("\nmodmax\f<_p>", -60, 60, 0, true, "Â°"))
		ctx.m_al	= new({id, "mod", "left"}, groups.angles:slider("Add \aCDCDCD60left/right\f<_p>", -60, 60, 0, true, "Â°"))
		ctx.m_ar	= new({id, "mod", "right"}, groups.angles:slider("\nar\f<_p>", -60, 60, 0, true, "Â°"))
		...
		ctx.d_d		= new({id, "body", "degree"}, groups.angles:slider("\nfdeg\f<_p>", -180, 180, 0, true, "Â°"))
		ctx.d_l		= new({id, "body", "left"}, groups.angles:slider("Range \aCDCDCD60left/right\nbodyl\f<_p>", -180, 180, 0, true, "Â°"))
		ctx.d_r		= new({id, "body", "right"}, groups.angles:slider("\nbodyr\f<_p>", -180, 180, 0, true, "Â°"))
		-- ctx.d_dbase	= groups.angles:slider("Foot base\f<_p>", -180, 180, 0, true, "Â°")
		-- ctx.d_dpow	= groups.angles:slider("Overdrive\f<_p>", 0, 50, 0, true, "Ď", 0.1)
		...
		ctx.dsp1	= new({id, "snap", "pitch_min"}, groups.other:slider("\f<_p>pmin", -89, 89, -45, true, "Â°"))
		ctx.dsp2	= new({id, "snap", "pitch_max"}, groups.other:slider("\f<_p>pmax", -89, 89, -45, true, "Â°"))
		ctx.dsy		= new({id, "snap", "yaw"}, groups.other:combobox("\f<p>Yaw\f<_p>", {"None", "Switch", "Random", "Spin", _AZAZI and "Đ°ĐˇĐ°ĐˇĐžĐš" or nil}))
		ctx.dsy1	= new({id, "snap", "yaw_min"}, groups.other:slider("\f<_p>ymin", 0, 360, 180, true, "Â°"))
		-- ctx.dsy2	= new({id, "snap", "yaw_max"}, groups.other:slider("\f<_p>ymax", -180, 180, 90, true, "Â°"))
	...
	-- 	vars.antiaim.builder[1][1]:set(pui.format "\vâ˘\r  Actions with \v" .. this.value)
	-- end, true)
end
...
do
	...
	callbacks.paint_ui:set(function ()
		...
		menu.main.bar:set("\a".. col:to_hex() .. "âžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâžâž")
	end)
...
local configs = {
	system = nil,
	default = "sakura::GS::",
	badge = pui.format("\vâ˘\r "),
	selected = 0, name = "",
	loaded = nil,
	list = {}
} do
	...
	actions.eval = function (raw, noparse)
		...
		local cheat, contents, pad = string.match(raw, "^sakura::(%a+)::([%w%+%/]+)(_*)")
	...
	actions.save = function (name, new)
		...
		local contents = string.format("(%s)[%s]{%s}", name, o_author or sakura.user.name, msgpack.pack(cfg))
		local encoded = string.gsub(base64.encode(contents), "[%+%/%=]", { ["+"] = "z113Z", ["/"] = "z143Z", ["="] = "_" })

		local ready = ("sakura::GS::%s"):format(encoded)
	...
	actions.import = function ()
		...
		local cfg = copied:match("^sakura::%a+::[%w%+%/]+_*")
	...
	local update = function (no_reval)
		...
		configs.name = configs.list[configs.selected]:gsub("^\a%x%x%x%x%x%x%x%xâ˘\a%x%x%x%x%x%x%x%x ", "")
	...
	trigger = function (event)
		...
		if (dists.mine and closest) and dists.mine < 40 or (closest == dists.mine and dists.mine < 128) then
			client.delay_call(totime(1), function ()
				client.fire_event("sakura::enemy_shot", {
...
antiaim.builder = {
	...
	modifier = function (self, scene)
		...
		elseif modifier == "ĐżĐľŃĐ´ĐľĐťŃŃĐ°" then
...
function generate_tag(str)
    local list = {}
    local spacer = "â "

    -- left to right buildup
    for i = 0, #str do
        local part = str:sub(1, i)
        local pad = spacer .. " "
        local count = #str - i
        table.insert(list, part .. "_" .. string.rep(pad, count))
    end

    -- separator
    table.insert(list, str .. " ")
    table.insert(list, str .. "_â ")
    table.insert(list, str .. " ")

    -- right to left breakdown
    for i = 1, #str do
        local pad = string.rep(spacer .. " ", i)
        local part = str:sub(i + 1)
        table.insert(list, pad .. "_" .. part)
    end

    table.insert(list, spacer .. " " .. spacer .. " " .. spacer .. " " .. spacer .. " ")
...
misc.filter = {
	callback = function (this)
		...
		cvar.con_filter_text:set_string(this.value and "sakura" or "")
	end,
	run = function (self)
...
logger.events = {
	...
	miss = function (shot)
		...
		local main, add = {
			...
			{"hc: ", {math.round(shot.hit_chance), "%%"}, (refs.rage.aimbot.hit_chance.value - shot.hit_chance > 3) and "âŽ" or ""} or nil,
			pre.difference and pre.difference ~= 0 and {"Î: ", {pre.difference, "t"}, pre.difference < 0 and "âŽ" or ""} or nil,
	...
	hit = function (shot)
		...
		local main, add = {
			...
			pre.difference ~= 0 and {"Î: ", {pre.difference, "t"}} or nil,
			(refs.rage.aimbot.hit_chance.value - shot.hit_chance > 5) and {"hc: ", {math.floor(shot.hit_chance), "%%"}, "âŽ"} or nil,
		}
...
logger.invent = function (event, main, add)
	...
	if event then
		...
		log.console[lc+1], log.console[lc+2] = col and col[1] or "", "â˘\r "
		log.screen[ls+1], log.screen[ls+2] = col and col[2] or "", "â˘\aE6E6E6\x02 "
	end
...
logger.run = function (self)
	vars.misc.logs.on:set_callback(function (this)
		...
		callbacks["sakura::enemy_shot"](this.value, self.events.evade)
...
local candies = database.read("sakura::candies") or 0
...
do
	...
	callbacks["sakura::enemy_shot"]:set(function (event)
	...
	callbacks["sakura::database_write"]:set(function (event)
...
do
	local user, cheat = sakura.user.name, _BLISS and "gamesense-bliss" or "gamesense"
	...
	ui.set_callback(menu.general.verify.auth.ref, function ()
		...
		http.get("https://backend.sakura.one/keygen", {
			headers = {
				["hst-uname"] = user,
				["hst-cheat"] = cheat,
			...
			if not s then sakura.print("Something went wrong. Try again later.") return end
			...
			if r.is_connected == "yes" then
				...
				sakura.print("You have already linked your discord")
			else
...
textures = {
	...
	local file_texture = readfile("sakura/sakura.png")
	local load_textures = function (data)
		textures.sakura = render.load_png(data, 1024, 1024)
	end

	if not file_texture then
		http.get("https://sakurax.testhosting.hu/main/sakura.png", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
				load_textures(raw.body)
				writefile("sakura/sakura.png", raw.body)
			end
		end)
	else
	...
	if _AZAZI then
		http.get("https://sakurax.testhosting.hu/azazogo/logo_lo.png", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
				textures.logo_l = render.load_png(raw.body, 35, 15)
			end
		end)
		http.get("https://sakurax.testhosting.hu/azazogo/logo_ro.png", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
...
local drag do
	...
	drag = {
		new = a(function (widget, props)
			...
			--
			callbacks["sakura::render_dpi"]:set(function (new)
...
{ -- logo mini
		0, x = 0, function (self, x, y)
			...
			local t = "      SAKURA  " .. ((not bfly and _LEVEL > 1) and colors.hexs .. string.format("%02x", render.get_alpha() * 255) .. string.upper(sakura.build) or "")
			...
			return vars.visuals.crosshair.style.value == "Logo", tw, th + 3
		end, desync = 0,
 	}, { -- dt
		0, x = 0, function (self, x, y)
...
hud.logs.preview, hud.logs.dummy = false, {
	{
		event = "hit",
		text = "\aA3D350\x01â˘\aE6E6E6\x02 Killed\aE6E6E6\x02 \aE6E6E6\x02\aE6E6E6\x01maj0r\aE6E6E6\x02 in \aE6E6E6\x02\aE6E6E6\x01head\aE6E6E6\x02\aE6E6E6\x02",
		time = math.huge,
		progress = {0},
	},
	{
		event = "miss",
		text = "\aA67CCF\x01â˘\aE6E6E6\x02 Missed\aE6E6E6\x02 \aE6E6E6\x01enQ\aE6E6E6\x02's\aE6E6E6\x01 head\aE6E6E6\x02 due to \aE6E6E6\x01unpredicted occasion",
		time = math.huge,
		progress = {0},
	},
	{
		event = "harm",
		text = "\ad35050\x01â˘\aE6E6E6\x02 Harmed by\aE6E6E6\x02 \aE6E6E6\x01enQ\aE6E6E6\x02 in \aE6E6E6\x01head\aE6E6E6\x02 for \aE6E6E6\x0172",
		time = math.huge,
		progress = {0},
	},
}
...
hud.keylist.paint = function (self, x, y, w, h)
	render.rectangle(x, y - 4, w, h -18, colors.accent, 4)
	render.blur(x, y, w, h)
...
if not _sigma then
	...
	welcome.render = function ()
		...
		local size = 400
		render.texture(textures.sakura, sc.x - size * 0.5, sc.y - size * 0.5, size, size, colors.accent:alphen(P2 * 255))
